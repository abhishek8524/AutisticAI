...
import { getLocations, getRankings } from '../services/api';
import './NonLoginMapView.css';

const QUICK_FILTERS = [
  { label: 'Quiet now', filter: 'quiet' },
...
const CATEGORY_CHIPS = [
  { icon: '/assets/icons/cat-library.svg', title: 'Quiet librar…', desc: 'Low noise n…', filter: 'library' },
  { icon: '/assets/icons/cat-cafe.svg', title: 'Soft-light c…', desc: 'Gentler lighti…', filter: 'cafe' },
  { icon: '/assets/icons/cat-park.svg', title: 'Outdoor ca…', desc: 'Open space …', filter: 'outdoor' },
  { icon: '/assets/icons/cat-explore.svg', title: 'Best place…', desc: 'Low crowds …', filter: null },
];

const FALLBACK_RANKINGS = [
  { name: 'Quiet Corner Library', tags: 'Very quiet • Soft lighting • …', score: 9.4, tier: 'high' },
  { name: 'Harbor Garden', tags: 'Open space • Gentle soun…', score: 9.0, tier: 'high' },
  { name: 'Fern Courtyard Café', tags: 'Moderate lighting • Quiet c…', score: 8.2, tier: 'medium' },
];

function NonLoginMapView({ onExploreMap }) {
          ...
          setRankings(
            data.slice(0, 3).map((loc) => ({
              ...
              tags: [
                loc.noiseLevel || '',
                loc.lightingLevel || '',
              ].filter(Boolean).join(' • ') || 'Sensory-friendly',
              score: loc.comfortScore ?? loc.comfort_score ?? 0,
              tier: (loc.comfortScore ?? loc.comfort_score ?? 0) >= 8.5 ? 'high' : 'medium',
            }))
          );
        }
      })
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (onExploreMap) {
      onExploreMap({ filter: activeFilter, searchQuery });
    }
  };

  const handleCategoryClick = (filter) => {
    if (onExploreMap) {
      onExploreMap({ filter, searchQuery: '' });
    }
  };
  ...
  const locationName = selectedLocation?.name || 'Quiet Corner Library';

  const quietnessScore = selectedLocation
    ? (10 - (selectedLocation.noise_score || 5)).toFixed(1)
    : '4.9';
  const lightingScore = selectedLocation
    ? ((selectedLocation.lighting_score || 5) / 10 * 5).toFixed(1)
    : '4.3';
  const spaceScore = selectedLocation
    ? (10 - (selectedLocation.crowd_score || 5)).toFixed(1)
    : '4.6';
  const overallScore = selectedLocation
    ? ((parseFloat(quietnessScore) + parseFloat(lightingScore) + parseFloat(spaceScore)) / 3).toFixed(1)
    : '4.8';
  const reviewCount = selectedLocation?.review_count || 128;
    ...
    <div className="nlm">
      ...
      <aside className="nlm-sidebar-left">
        ...
        <div className="nlm-logo-row">
          ...
          <button className="nlm-settings-btn" aria-label="Settings">
          ...
          <div className="nlm-ranked-list">
            {rankings.map((place) => (
              <div
                ...
                role="button"
                tabIndex={0}
                onClick={() => handleLocationSelect({ name: place.name, comfort_score: place.score })}
                onKeyDown={(e) => e.key === 'Enter' && handleLocationSelect({ name: place.name, comfort_score: place.score })}
              >
      ...
      <aside className="nlm-sidebar-right">
        ...
        <div className="nlm-ratings-card">
          ...
          <div className="nlm-stars">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="nlm-star filled">
                <div className="star-overlay" />
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M8 1.33L10.06 5.51L14.67 6.18L11.33 9.43L12.12 14.01L8 11.85L3.88 14.01L4.67 9.43L1.33 6.18L5.94 5.51L8 1.33Z" fill="#F5A623" stroke="#F5A623" strokeWidth="0.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
            ))}
            <div className="nlm-star empty">
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M8 1.33L10.06 5.51L14.67 6.18L11.33 9.43L12.12 14.01L8 11.85L3.88 14.01L4.67 9.43L1.33 6.18L5.94 5.51L8 1.33Z" fill="none" stroke="#CBD5E1" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
          </div>
          ...
          <div className="nlm-rating-tags">
            <span className="nlm-rating-tag">92% would revisit</span>
            <span className="nlm-rating-tag">Low-noise favorite</span>
            <span className="nlm-rating-tag">Best before 11am</span>
          </div>

          {/* Score Bars */}
          <div className="nlm-score-bars">
            <div className="nlm-score-row">
              <span className="nlm-score-label">Quietness</span>
              <div className="nlm-score-bar">
                <div className="nlm-score-bar-fill" style={{ width: `${(parseFloat(quietnessScore) / 5) * 100}%` }} />
              </div>
              <span className="nlm-score-value">{quietnessScore}</span>
            </div>
            <div className="nlm-score-row">
              <span className="nlm-score-label">Lighting</span>
              <div className="nlm-score-bar">
                <div className="nlm-score-bar-fill" style={{ width: `${(parseFloat(lightingScore) / 5) * 100}%` }} />
              </div>
              <span className="nlm-score-value">{lightingScore}</span>
            </div>
            <div className="nlm-score-row">
              <span className="nlm-score-label">Space</span>
              <div className="nlm-score-bar">
                <div className="nlm-score-bar-fill" style={{ width: `${(parseFloat(spaceScore) / 5) * 100}%` }} />
              </div>
              <span className="nlm-score-value">{spaceScore}</span>
            </div>
          </div>
        </div>
